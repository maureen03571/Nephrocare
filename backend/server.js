...
app.get('/api/patient/:id/link-code', (req, res) => {
  ...
  if (!user || user.role !== 'patient') return res.status(404).json({ success: false, message: 'Patient not found' });
...
app.post('/api/caregiver/link-patient', (req, res) => {
  const { caregiverId, linkCode } = req.body;
  const caregiver = dataStore.users.find(u => u.id === caregiverId && u.role === 'caregiver');
  if (!caregiver) return res.status(404).json({ success: false, message: 'Caregiver not found' });
  
  const patient = dataStore.users.find(u => u.role === 'patient' && u.linkCode === linkCode);
...
app.get('/api/caregiver/:id/patient', (req, res) => {
  const caregiver = dataStore.users.find(u => u.id === req.params.id);
  if (!caregiver || !caregiver.patientId) return res.status(404).json({ success: false, message: 'No patient linked' });
  
  const patient = dataStore.users.find(u => u.id === caregiver.patientId);
  const dashboard = getPatientDashboard(patient.id);
...
app.get('/api/education/articles', (req, res) => {
  ...
  // Selection logic: 2 medical, 1 nutrition, 1 lifestyle, 1 management
  const select = (list, seed) => list[seed % list.length];
  
  const articles = [
    select(ARTICLES_BY_CATEGORY.medical, week),
    select(ARTICLES_BY_CATEGORY.medical, week + 1), // Pick another medical article
